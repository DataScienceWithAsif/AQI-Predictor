...
[![Hourly Feature Pipeline](https://github.com/DataScienceWithAsif/AQI-Predictor/actions/workflows/feature_pipeline.yml/badge.svg?branch=main)](https://github.com/DataScienceWithAsif/AQI-Predictor/actions/workflows/feature_pipeline.yml)
[![Daily Training Pipeline](https://github.com/DataScienceWithAsif/AQI-Predictor/actions/workflows/training_pipeline.yml/badge.svg?branch=main)](https://github.com/DataScienceWithAsif/AQI-Predictor/actions/workflows/training_pipeline.yml)
...
| **Testing** | pytest-based pipeline test suites (30+ tests) + standalone post-training checks |
...
<details>
<summary><b>📈 Full EDA charts</b> (click to expand — generated by <code>pipelines/EDA/eda.py</code>)</summary>

Run `python pipelines/EDA/eda.py` to regenerate all charts against live data:
...
SHAP (Permutation Explainer) is computed for the winning model at each horizon, both saved as report-ready plots (`pipelines/training_pipeline/day6_shap_plots/`) and surfaced live in the dashboard's "What drives this prediction?" panel.
...
Core pipeline logic — API parsing, feature engineering, and model training — is covered by executed pytest suites:
...
| `train.py` | Date-based split leakage prevention, metric calculations, winner-selection logic, multi-city one-hot encoding, SHAP integration |
| `test_predictions.py` | Post-training sanity checks (plausible ranges, baseline comparison, directional sensitivity, deterministic inference) |

**30+ pytest tests** across pipeline modules, plus standalone post-training checks via `python pipelines/training_pipeline/test_predictions.py`.
...
│   │   ├── daily_features.py
│   │   ├── final_features.py
│   │   ├── get_coords.py            # Shared geocoding helper
│   │   ├── hopsworks_io.py
│   │   ├── hopsworks_io2.py
...
# create a .env file in the repo root and add required keys:
# OPENWEATHER_KEY=...
# AQICN_TOKEN=...
# HOPSWORKS_API_KEY=...
